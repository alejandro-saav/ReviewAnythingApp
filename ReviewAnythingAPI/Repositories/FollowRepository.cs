...
public class FollowRepository : Repository<Follow>, IFollowRepository
{
    ...
    {
        return await _context.Follows.AsNoTracking().Where(f => f.FollowerUserId == userId).Select(f => f.Follower!).ToListAsync();
    }

    public async Task<IEnumerable<int>> GetUserFollowingsIdsAsync(int userId)
    {
        return await _context.Follows.AsNoTracking().Where(f => f.FollowerUserId == userId).Select(f => f.FollowingUserId!.Value)
            .ToListAsync();
    }
