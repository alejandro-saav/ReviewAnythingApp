...
namespace BlazorApp1.Components.Pages.LikedReviews;
...
public partial class LikedReviews : ComponentBase
{
    ...
    private async Task FetchReviews()
    {
        try
        {
            IsLoading = true;
            StateHasChanged();
            var reviews = await ReviewService.GetLikesReviewsAsync(QueryParams);
            if (reviews.Any())
            {
    ...
    {
        var uri = PaginationHelper.BuildUriFromParams("/likedreviews", QueryParams);
    ...
    private void NavigatePage(int pageNumber)
    {
        ...
        Navigation.NavigateTo($"/likedreviews{pageParams}", forceLoad: true);
    }
}